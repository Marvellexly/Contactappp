...
class AppPage(tk.Frame):
	...
	def create_left_frame(self):
		self.left_frame = tk.Frame(self, bg=self.bg) # 1
	...
	def create_right_frame(self):
		self.right_frame = tk.Frame(self, bg=self.bg, width=2*self.settings.width//3) # 2
	...
	def create_left_header(self):
		...
		self.left_header = tk.Frame(self.left_frame, width=frame_w, height=frame_h//5, bg=self.bg)
		...
		self.searchbox_frame = tk.Frame(self.left_frame, width=frame_w, height=frame_h//20, bg=self.bg)
		self.searchbox_frame.pack(fill="x")

		self.entry_search_var = tk.StringVar()
		self.entry_search = tk.Entry(self.searchbox_frame, font=("Arial", 12), textvariable=self.entry_search_var)
		self.entry_search.grid(row=0, column=0, sticky="nsew")

		self.button_search = tk.Button(self.searchbox_frame, font=("Arial", 12), text="Find", command=self.clicked_search_btn)
	...
	def create_left_content(self):
		...
		self.left_content = tk.Frame(self.left_frame, width=frame_w, height=4*frame_h//5, bg=self.bg)
		self.left_content.pack(fill="x")

		self.contacts_listbox = tk.Listbox(self.left_content, bg=self.bg, fg=self.fg , height=4*frame_h//5, font=("Arial", 12))
	...
	def clicked_item_list_box(self, event):
		if not self.update_mode:
			...
			self.table_info[3][1].configure(text=Obat)
			self.table_info[4][1].configure(text=Exp_date)
	...
	def create_right_header(self):
		...
		self.right_header = tk.Frame(self.right_frame, width=frame_w, height=frame_h, bg=self.bg)
	...
	def create_detail_right_header(self):
		...
		self.detail_header = tk.Frame(self.right_header, width=frame_w, height=frame_h, bg=self.bg)
		...
		self.fullName_label = tk.Label(self.detail_header, text=fullName, font=("Arial", 26), bg=self.bg, image=self.virt_fullName_image, compound="c", width=frame_w, height=frame_h, fg=self.fg)
	...
	def create_detail_right_content(self): 
		...
		for numberPhone, info in self.current_contact.items():
			info = [
				...
				['Email :', info['email']],
				['Obat :', info['Obat']],
				['Exp date :', info['Exp_Date']]
			]
		rows, columns = len(info), len(info[0])
		for row in range(rows):
			aRow = []
			for column in range(columns):
				label = tk.Label(self.detail_content, text=info[row][column], font=("Arial", 12), bg=self.bg, fg=self.fg)
				if column == 0:
					sticky = 'e'
				else:
					sticky = 'w'
	...
	def create_detail_right_footer(self):
		...
		for feature in features:
			button = tk.Button(self.detail_footer, text=feature, bg=self.bg, fg=self.fg, font=("Arial", 12, "bold"), bd=0)
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20, pady=30)
	...
	def clicked_update_btn(self):
		...
		for row in range(rows):
			...
			for column in range(columns):
				if column == 0:
					label = tk.Label(self.detail_update_content,text=info[row][column], font=("Arial", 12), bg=self.bg, fg=self.fg)
					sticky = 'e'
					label.grid(row=row, column=column, sticky=sticky)
					aRow.append(label)
				else:
					...
					entry = tk.Entry(self.detail_update_content,font=("Arial", 12), bg=self.bg, textvariable=self.entry_update_contact_vars[row])
		...
		self.buttons_features = []
		for feature in features:
			button = tk.Button(self.detail_update_footer, text=feature, bg=self.bg, fg=self.fg, font=("Arial", 12, "bold"), bd=0, command=commands[features.index(feature)])
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20, pady=30)
			self.buttons_features.append(button)
	...
	def clicked_delete_btn(self):
		...
		confirm = messagebox.askyesnocancel("Contactapp Confirmation", "Are you sure to delete this contact ?")
		index = self.last_current_index
		if confirm:
	...
	def clicked_add_new_btn(self):
		...
		self.detail_add_new_header = tk.Frame(self.right_header, width=frame_w, height=frame_h, bg="white")
		self.detail_add_new_header.grid(row=0, column=0, sticky="nsew")

		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.title_header_label = tk.Label(self.detail_add_new_header, text="Tambah Kontak Baru", font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound="c", bg="white")
		...
		features = ['Save', 'Cancel']
		commands = [self.clicked_save_add_new_btn, self.clicked_cancel_btn]
		self.buttons_features = []
		for feature in features:
			index = features.index(feature)
			button = tk.Button(self.detail_add_new_footer, text=feature, bg="white", fg="black", font=("Arial", 12, "bold"), bd=0, command=commands[index]) #, command=commands[features.index(feature)]
	...
	def clicked_save_add_new_btn(self): 
		...
		confirm = messagebox.askyesnocancel("Contactapp Confirmation", "Are you sure to add this contact ?")
		...
		if confirm:
			...
			Obat = self.entry_update_contact_vars[5].get()
			Exp_Date = self.entry_update_contact_vars[6].get()
			new_contact = {
				phone : {
					'f_name' : f_name,
					'l_name' :l_name,
